package com.kumbirai.udemy.broker.config;
...
public class BrokerConfig
{
	...
	public static BrokerConfig from(final JsonObject config)
	{
		...
		{
			throw new ConfigException(ConfigLoader.SERVER_PORT + " not configured!");
		}
		final String version = config.getString("version");
		if (Objects.isNull(version))
		{
			throw new ConfigException("version is not configured in config file!");
		}
